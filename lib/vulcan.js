...
var Vulcan = function Vulcan(opts) {
  ...
  this.enableSourcemaps = Boolean(opts.sourcemaps);
  this.sourcemapRoot = opts.sourcemapRoot === undefined ? process.cwd() : path.resolve(opts.sourcemapRoot);
...
Vulcan.prototype = {
  ...
  flatten: function flatten(tree, isMainDoc) {
    ...
    if (this.enableSourcemaps) {
      var inlineScripts = dom5.queryAll(doc, matchers.JS_INLINE);
      inlineScripts.forEach(function (script) {
        var content = dom5.getTextContent(script);
        var sourceMap = this.createIdentitySourcemap(path.relative(this.sourcemapRoot, tree.href), content,
            script.__location.startTag.line - 1,
            script.__location.startTag.col - 1 + script.__location.startTag.endOffset -
                script.__location.startTag.startOffset,
            false);

        var base64Map = new Buffer(sourceMap.toString()).toString('base64');
  ...
  inlineScripts: function inlineScripts(doc, href) {
    ...
    var createSourcemaps = this.enableSourcemaps;
    var scriptPromises = scripts.map(function(script) {
      ...
      return this.loader.request(uri).then(function(content) {
        if (content) {
          if (createSourcemaps) {
            content = '\n' + content;
            var relativeUri = path.relative(this.sourcemapRoot, uri);
            var sourceMap = this.createIdentitySourcemap(relativeUri, content, -1, 0, true);
  ...
  createIdentitySourcemap: function createIdentitySourcemap(relativeUri, content, originalLineOffset,
      firstLineColumnOffset) {

    var sourceMap = new sourcemaps.SourceMapGenerator();

    var tokenizer = acorn.tokenizer(content, {locations: true});
    var token;
    while ((token = tokenizer.getToken()).type.label !== "eof") {
      var originalLine = token.loc.start.line + originalLineOffset;
      var mapping = {
        // generated is the relative position in the script tag
        generated: {
        ...
        original: {
          line: originalLine,
          column: token.loc.start.column +
              (token.loc.start.line !== originalLineOffset + 1 ? 0 : firstLineColumnOffset)
        },
        source: relativeUri
      };
  ...
  // Update sourcemaps with new line offsets
  updateSourcemapLocations: function updateSourcemapLocations(doc, href) {
    // We need to serialize and reparse the dom for updated location information
    var newDoc = parse5.parse(parse5.serialize(doc), {locationInfo: true});

    var scripts = dom5.queryAll(newDoc, matchers.JS_INLINE);
    ...
    scripts.forEach(function(script) {
      ...
      var updatedMap = new sourcemaps.SourceMapGenerator();
      ...
      originalMap.eachMapping(function(mapping) {
        var newMapping = {
          ...
          generated: {
            line: mapping.generatedLine + script.__location.startTag.line - 1,
            column: mapping.generatedColumn +
                (!(mapping.generatedLine === 1 && /\S/.test(contentLines[0])) ? 0 :
                    script.__location.startTag.col - 1 +
                    script.__location.startTag.endOffset -
                    script.__location.startTag.startOffset)
          }
        };
        ...
        updatedMap.addMapping(newMapping);
      });

      var base64Map = new Buffer(updatedMap.toString()).toString('base64');
      content = content.replace(sourceMapCommentExpr,
           '\n//# sourceMappingURL=data:application/json;charset=utf8;base64,' +
           base64Map + '\n');
  ...
  _process: function _process(target, cb) {
    ...
    if (this.enableSourcemaps) {
      chain = chain.then(function(docObj) {
        return this.updateSourcemapLocations(docObj.doc, docObj.href);
      }.bind(this));
    }
    chain.then(function(docObj) {
      cb(null, parse5.serialize(docObj.doc));
    }).catch(cb);
  },

  process: function vulcanProcess(target, cb) {
    if (this.inputUrl) {
      ...
      if (this.abspath) {
        ...
        target = this.pathResolver.pathToUrl(path.relative(process.cwd(), target));
      }
